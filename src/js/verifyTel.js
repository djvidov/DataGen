...
function verifyPhoneNumber(phoneNumberStr){
    ...
    var totalNumbers = justNumber.length;
    var extractedISDcode1 = justNumber.substring(0, totalNumbers - 10); // cod ISD cu un caracter
    var extractedISDcode2 = justNumber.substring(0, totalNumbers - 9); //// cod ISD cu 2 caractere
    ...
    if (totalNumbers >= 8 && totalNumbers <= 16) { //majoritatea numerelor de telefon au intre 8 si 16 caractere (cu prefix international inclus)
        if (ISDcodes.includes(parseInt(extractedISDcode1)) || ISDcodes.includes(parseInt(extractedISDcode2)) ) { // verifica pt cod international
            if(getRouPrefix.match(rouMobilePrefix)){ //pt Romania
                return "valid 1";
            }
            else{
                return "invalid 1";
